...
find_rows_query(df3, (['league_name', 'gender'], ['English Premier League', 'Male']),
                llista_columnes)

find_rows_query(df3, (['overall', 'age'], [(72, 90), (18, 25)]), llista_columnes)

columnes2 = ['short_name', 'year', 'overall', 'league_name', 'gender', 'age']
find_rows_query(df3, (['league_name', 'age'], ['English Premier League', (18, 25)]), columnes2)

find_rows_query(df3, (['age', 'league_name'], [(18, 25), 'English Premier League']), columnes2)
...
d = {41: {'short_name': ['Iniesta', 'Iniesta', 'Iniesta'], 'overall': [88, 88, 87],
...
def top_average_column(data: dict, identifier: str, col: str) -> list:
    """Inputs:
    ...
    Per a cada sofifa_id, calculi el valor mitjà de la característica col
    si teniu informació de threshold o més anys. Si no, s'ignora aquest sofifa_id.
    Si algun element de la llista té el valor NaN, també s'ignora aquest sofifa_id.
    ...
    res = []
    resfin = []
    for nums, params in data.items():
        res.append(params[identifier])
        params[col] = [int(i) for i in params[col]]
        res.append(round(statistics.mean(params[col]), 3))
        res_dic['value'] = params[col]
        res_dic['year'] = params['year']
        res.append(res_dic)
        tupla = tuple(res)
        resfin.append(tupla)
        print(resfin)
...
def best_defense(df: pd.DataFrame, position):
    ...
    df['best_center_backs'] = (d*df.defending + c*df.mentality_interceptions +
                               d*df.defending_sliding_tackle +
                               d*df.defending_standing_tackle +
    ...
    df['best_wb'] = (d*df.defending + b*df.skill_ball_control + a*df.dribbling +
                     ...
                     c*df.skill_long_passing + d*df.movement_reactions +
                     d*df.movement_agility +
    ...
    if position == "C":
        #DEFENSOR CENTRAL:
        plt.figure(figsize=(15, 6))
        sd3_central = center_back.sort_values('best_center_backs',
                                              ascending=False)[:10]
