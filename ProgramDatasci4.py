...
def join_male_female(filepath: str, year: int) -> pd.DataFrame:
    ...
    - gender: 'Male' or 'Female'
    - year of the data"""
    df_male = read_add_year_gender(filepath, 'M', year)
    df_fem = read_add_year_gender(filepath, 'F', year)
    joined_result = df_male.append(df_fem)
...
alw = list(range(2016, 2023))
tm_all = join_datasets_year('data/', alw)

tp_A = ['long_name', 'player_positions', 'age']
find_max_col(tm_all, 'age', tp_A)

tp_B = ['short_name', 'year', 'overall']
find_max_col(tm_all, 'short name', tp_B)
...
columnes_q = ['short_name', 'year', 'overall', 'league_name', 'gender']

find_rows_query(tm_all, (['league_name', 'gender'], ['English Premier League', 'Male']),
                columnes_q)

find_rows_query(tm_all, (['overall', 'age'], [(72, 90), (18, 25)]), columnes_q)

find_rows_query(tm_all, (['league_name', 'age'], ['English Premier League', (18, 25)]),
                columnes_q)

find_rows_query(tm_all, (['age', 'league_name'], [(18, 25), 'English Premier League']),
                columnes_q)
...
exer2 = ['short_name', 'year', 'age', 'overall', 'potential']

belg_fins24 = tm_all[exer2][(tm_all['age'] < 25)&(tm_all['nationality_name'] == 'Belgium')&
                            (tm_all['gender'] == 'Male')]
...
tm_all[exer2][(tm_all['gender'] == 'Female')&(tm_all['age'] > 28)&
              (df_all['overall'] > 85)]

height = tm_all['height_cm'].astype(float)/100
weight = tm_all['weight_kg'].astype(float)
tm_all['BMI'] = round(weight/(height*height), 3)
exer2.append('BMI')

tm_all[(tm_all['year'] == 2016)&(tm_all['gender'] == 'Male')]
...
calculate_BMI(tm_all, 'Male', 2016, llista_col)
...
df_BMI = calculate_BMI(df_all, 'Male', 2022, ['club_flag_url'])
df_BMI['pais'] = df_BMI['club_flag_url'].str.extract('flags/([a-zA-Z]+(?:-[a-zA-Z]+)*).png')
df_BMI
# gràfica
df_BMI2 = df_BMI.groupby('pais').BMI.agg(['max'])
ax = df_BMI2.plot(kind='bar', figsize=(16, 6), title='BMI max', rot=40)
plt.show()

llista_col = ['age']
data = calculate_BMI(df_all, 'Female', 2020, llista_col)
...
data.loc[(data['BMI'] >= 18.5) & (data['BMI'] < 25),
         'Adult body mass index'] = "Normal weight (18.5 kg/m2 <= BMI < 25 kg/m2)"
data.loc[(data['BMI'] >= 25) & (data['BMI'] < 30),
         'Adult body mass index'] = "Overweight (25 kg/m2 <= BMI < 30 kg/m2)"
data

# Plot dades dataset
data3 = data[['BMI', 'Age',
              'Adult body mass index']].groupby(['Age', 'Adult body mass index']).mean().unstack()
...
data22 = data2[['Age', 'Adult body mass index',
                'Total']].groupby(['Age', 'Adult body mass index']).mean().unstack()
...
prova1 = {41: {'short_name': ['Iniesta', 'Iniesta', 'Iniesta'], 'overall': [88, 88, 87],
               'potential': [88, 88, 87], 'player_positions': ['CM', 'CM', 'CM, LM'],
               'year': [2016, 2017, 2018]}}

clean_up_players_dict(prova1, col_query)

# Prova2
ids = [261799, 158023, 20801, 41]
new_cols = ['short_name', "player_positions", 'year', 'age', 'overall', 'potential']
prova2 = players_dict(df_all, ids, new_cols)
col_query = [("short_name", "del_rep"), ('potential', 'one')]
clean_up_players_dict(prova2, col_query)
...
df16_18 = join_datasets_year('data/', years)
colstodic = ["short_name", "overall", "potential", "player_positions", "year"]
dct9 = players_dict(df16_18, ids, colstodic)
# La query que passaríeu a la funció de l'apartat 4b per netejar aquest diccionari:
col_query = [("short_name", "del_rep"), ('potential', 'one')]
# El diccionari “net”;
clean_up_players_dict(dct9, col_query)
